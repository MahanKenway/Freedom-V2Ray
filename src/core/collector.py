...
class ConfigCollector:
    ...
    def __init__(
        ...
        max_workers: int,
        fetch_timeout: float,
        source_health: SourceHealthManager | None = None,
    ) -> None:
        self.sources = list(sources)
        self.tester = tester
        self.max_workers = max_workers
        self.fetch_timeout = fetch_timeout
        self.source_health = source_health or SourceHealthManager()
    ...
    def collect(self) -> Dict[Protocol, List[Config]]:
        ...
        prioritized_sources = self.source_health.prioritize(self.sources)

        for source in prioritized_sources:
            configs = self.fetch_source(source)
            logger.info("Fetched %s configs from %s", len(configs), source)
            all_configs.extend(configs)
            self.source_health.record_result(source, had_configs=bool(configs))
